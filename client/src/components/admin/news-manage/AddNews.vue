<template>
  ...
  >
    <template v-slot:default="{}">
      <v-card>
        <v-toolbar color="primary" title="Thêm mới tin tức"></v-toolbar>
        <v-card-text>
          <v-form ref="form">
            <v-row class="mt-2">
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :label="imageResponse ? 'Thay đổi ảnh' : 'Chọn ảnh hiển thị'"
                  variant="outlined"
                  hide-details="auto"
                  density="compact"
...
export default {
  ...
  methods: {
    async submit() {
      ...
      try {
        if (valid) {
          const res = await this.$axios.post(`${this.$gAPI}/News/Store`, {
            ...this.neww,
          });
          if (res.status === 200) {
            this.$emit("toggle");
  ...
  watch: {
    open(newVal) {
      if (!newVal) {
        ...
        this.neww.author = "";
        this.neww.imagePath = "";
        this.neww.shortDescription = "";
        this.neww.description = "";
