...
<script>
import AddCategory from "@/components/admin/category-manage/AddCategory.vue";
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: "Mã loại sản phẩm ", key: "categoryId", sortable: false },
      { title: "Loại sản phẩm", key: "categoryName", sortable: false },
      // { title: "Icon", key: "icon", sortable: false },
      // { title: "Mô tả (Ghi chú)", key: "desciption", sortable: false },
      {
        title: "Chức năng",
        key: "action",
        sortable: false,
    ...
    async getData() {
      try {
        const res = await this.$axios.get(
          `http://localhost:9091/api/v1/category/getAll`
        );
    ...
    async confirmDelete() {
      try {
        const res = await this.$axios.delete(
          `http://localhost:9091/api/v1/category/delCate/${this.dataDetail.categoryId}`
        );
        if (res.status === 200) {
