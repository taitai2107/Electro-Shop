...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: "Mã ng dùng", key: "accountId", sortable: false },
      ...
      { title: "Phân quyền", key: "roleId", sortable: false },
  ...
  methods: {
    ...
    async getData() {
      try {
        const res = await this.$axios.get(
          `${this.$gAPI}/Account/ListByRole?roleId=user`
        );
        this.data = res.data;
      } catch (err) {
        console.log(err);
    ...
    async confirmDelete() {
      try {
        if (
          this.dataDetail.userName !== "admin" &&
          this.dataDetail.userName !== "user"
        ) {
          const res = await this.$axios.delete(
            `${this.$gAPI}/Account/Delete/${this.dataDetail.accountId}`
          );
          if (res.status === 200) {
